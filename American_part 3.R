...
new_data = read_csv("bird tracking data.csv")
...
filename <- "Black-crowned night heron sun position.csv" 
write.csv(p1, file = filename, row.names = FALSE)
...
filename <- "Black_bellied_plover sun position.csv" 
write.csv(p2, file = filename, row.names = FALSE)
...
tail(Brown_pelican)
p3 = na.omit(Brown_pelican)

filename <- "Brown_pelican sun position.csv" 
write.csv(p3, file = filename, row.names = FALSE)
...
Long_billed_curlew = na.omit(Long_billed_curlew)
filename <- "Long_billed_curlew sun position.csv" 
write.csv(Long_billed_curlew, file = filename, row.names = FALSE)
...
tail(Pacific_loon)
Pacific_loon = na.omit(Pacific_loon)

filename <- "Pacific_loon sun position.csv" 
write.csv(Pacific_loon, file = filename, row.names = FALSE)
...
filename <- "Swainsons_hawk sun position.csv" 
write.csv(Swainsons_hawk, file = filename, row.names = FALSE)
...
filename <- "Anerucan Bird for Moon position.csv"  # Specify the filename and path as desired
...
bird_data = data.frame(bird = dff$species ,  lon = dff$longitude , lat = dff$latitude , direction= dff$theta)
head(bird_data)
